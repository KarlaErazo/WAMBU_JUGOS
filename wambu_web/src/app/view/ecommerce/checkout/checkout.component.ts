...
// 
  ...
  public gestionFactura(path, idDetallePago) {
    ...
    var factura = {
      ...
      descuento: 0,
      ice: 0,
      iva: this.valores.iva.toFixed(2),
      total: this.valores.total.toFixed(2),
   ...
   /*  var query = '';
    ...
    for (let carrito of this.itemsCarrito.lstCarrito) {
      ...
      var datos = {
        ...
        subtotal:this.obtenerTotalItem(carrito.cantidad, carrito.data),
        idFactura:idFactura,
        idProducto:carrito.data.idProducto
      }

      this.conexion.post("gestionFacturaDetalle", this.usuario.token, datos).subscribe(
        (res: any) => {
          indice += 1;
          console.log(res);
        },
    ...
    setTimeout(() => {
      if(indice == this.itemsCarrito.lstCarrito.length){
        this.toastr.success("Gracias Por Su Compra, Inmediatamente Nos Communicaremos, Esta Factura Esta Agregada Exitosamente", "Gestión Factura");
        setTimeout(() => {          
          this.enviaEmail();
        }, 1500);        
        this.cart.eliminarCarrito();
      }else{
        this.toastr.warning("El detalle de los productos no se guardó correctamente.", "Gestión Detalle Factura");
      }  
    }, 3000);
  ...
  public enviaEmail(){    
    ...
    var datos : any = {
      asunto : 'COMPRA EXITOSA JUGOS BAMBU',
      nombre : this.formEmail.nombre,
      correo : this.formEmail.correo,    
      telefono : this.formEmail.telefono
    }
      ...
      if (this.comprobante.files.length > 0) {
        ...
        this.conexion.upload("obtenerPath", form).subscribe(
          (res: any) => {
            console.log(res)
            this.spinner.hide();
            this.gestionDetallePago('TRANSFERENCIA', res.resultado);
          },
          err => {
            this.spinner.hide();
            console.log(err);
  ...
  public verCarrito() {
    ...
    console.log(this.itemsCarrito.lstCarrito);
